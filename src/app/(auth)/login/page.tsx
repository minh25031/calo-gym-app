  ...
  return (
    <div className="w-full max-w-sm space-y-6">
      <div className="text-center">
        ...
        <h1 className="text-2xl font-bold text-[var(--text)]">Chào mừng trở lại</h1>
        <p className="text-sm text-[var(--text-muted)] mt-1">Đăng nhập vào CaloGym</p>
      </div>

      {verified === "pending" && (
        <div className="rounded-xl bg-blue-500/10 border border-blue-500/20 p-3 text-sm text-blue-400 text-center">
      ...
      <form action={action} className="space-y-4">
        ...
        <div>
          ...
          <input name="password" type="password" required placeholder="••••••••" className="mt-1.5" />
        </div>
      ...
      <p className="text-center text-sm text-[var(--text-muted)]">
        Chưa có tài khoản? <Link href="/register" className="text-[var(--primary)] hover:underline font-medium">Đăng ký</Link>
      </p>
    </div>
  )
}
...
export default function LoginPage() {
  return (
    <div className="flex min-h-dvh items-center justify-center p-4 bg-[var(--bg)]">
      ...
      <Suspense fallback={<div className="text-[var(--text-muted)]">Đang tải...</div>}>
        <LoginForm />
      </Suspense>
    </div>
  )
